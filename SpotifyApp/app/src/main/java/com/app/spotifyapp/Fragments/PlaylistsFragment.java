...
public class PlaylistsFragment extends Fragment {
    ...
    public void onStart() {
        ...
        /*new Handler().postDelayed(() -> {
            api.AddToPlaylist(MainActivity.getAccessToken(), "7hnrA0Ngd2yNhRdHSWYklL", new String[]{"spotify:track:42GcjriRK6srwHkfbkBqVl", "spotify:track:3RKblQNJs8HPvWBgXmq6nn"});
        }, 1000);*/

     /*   new Handler().postDelayed(() -> {
            api.RemoveFromPlaylist(MainActivity.getAccessToken(), "7hnrA0Ngd2yNhRdHSWYklL", new String[]{"spotify:track:42GcjriRK6srwHkfbkBqVl", "spotify:track:3RKblQNJs8HPvWBgXmq6nn"});
        }, 1000);*/
    ...
    private void GetPlaylists(){
        ...
        AlbumsRecyclerViewAdapter adapter = new AlbumsRecyclerViewAdapter(requireContext(), playlists, position -> {
            Bundle bundle = new Bundle();
            bundle.putString("playlistID", playlists.get(position).Uri);
            Navigation.findNavController(requireView()).navigate(R.id.action_playlists_to_playlistTrackListFragment, bundle);

            Toast.makeText(requireContext(), playlists.get(position).Name, Toast.LENGTH_LONG).show();
        });
